...
export default function Skeleton({category = true, cantidad = 4}){
  ...
  const cards = (num) => {
    ...
    for (let i = 0; i < num; i++) {
      cod.push(
        <div className="w-72 h-96 rounded-lg bg-white p-3 shadow ">
            <div className=" w-full h-56 rounded-lg object-cover object-center bg-gray-200">
             
          </div>
          <a>
            <p className="my-4 pl-4 h-12 rounded-lg bg-gray-200 text-gray-200 select-none"></p>
          </a>
          <p className="mb-4 text-xl rounded-lg bg-gray-200 text-gray-200 select-none">a</p>
      </div>
      )
    }
